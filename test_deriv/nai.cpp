...
#ifdef ORDER1
void hgp_os_nai_d1(const LInt& LCode, const UInt& inp2, const UInt& nAtoms, 
		const Double* icoe, const Double* iexp, const Double* iexpdiff, const Double* ifac, 
		const Double* P, const Double* A, const Double* B, const Double* N, 
		const UInt* Z, Double* abcd);
#endif

#ifdef ORDER2
void hgp_os_nai_d2(const LInt& LCode, const UInt& inp2, const UInt& nAtoms, 
		const Double* icoe, const Double* iexp, const Double* iexpdiff, const Double* ifac, 
		const Double* P, const Double* A, const Double* B, const Double* N, 
		const UInt* Z, Double* abcd);
#endif
...
void nai::directnai_d1(const Int& Li, const Int& Lj, const Int& natoms, 
		...
		for(Int iBas=0; iBas<nBra1Bas; iBas++) {
			...
			for(Int jp=0; jp<jnp; jp++) {
				for(Int ip=0; ip<inp; ip++) {
					...
					Double tmp_r2 = nuclearIntegral(pref, alpla, N, AN, PAx, PAy, PAz, PBx, PBy, PBz,  
							Px, Py, Pz, l1, m1, n1, l2, m2, n2, natoms, isSameCenter);
...
void nai::directnai_d2(const Int& Li, const Int& Lj, const Int& natoms, 
		const Int& inp, const Double* icoe, const Double* iexp, const Double* A, 
		const Int& jnp, const Double* jcoe, const Double* jexp, const Double* B, 
		const Double* N, const Int* AN, const Int& pos, const Int& dir, Double* abcd)
{
		...
		for(Int iBas=0; iBas<nBra1Bas; iBas++) {
			...
			for(Int jp=0; jp<jnp; jp++) {
				for(Int ip=0; ip<inp; ip++) {
					...
					Double tmp_r1 = nuclearIntegral(pref, alpla, N, AN, PAx, PAy, PAz, PBx, PBy, PBz,  
							Px, Py, Pz, l1, m1, n1, l2, m2, n2, natoms, isSameCenter);
					...
					Double r3 = 0.0E0;
					if (newL>1) {
						Double tmp_r3 = nuclearIntegral(pref, alpla, N, AN, PAx, PAy, PAz, PBx, PBy, PBz,  
								Px, Py, Pz, l1, m1, n1, l2, m2, n2, natoms, isSameCenter);
					...
					Double tmp_r4 = nuclearIntegral(pref, alpla, N, AN, PAx, PAy, PAz, PBx, PBy, PBz,  
							Px, Py, Pz, l1, m1, n1, l2, m2, n2, natoms, isSameCenter);
...
void nai::nai_test(const string& derivFile, const Int& maxL, const Int& order,
		...
		const vector<Double>& N, const vector<Int>& atomicNumbers)
{
	...
	for(Int i=0; i<(Int)atomicNumbers.size(); i++) {
		Z[i] = atomicNumbers[i];
	}
	...
	for(Int jpos=0; jpos<MAX_SHELL_TYPES; jpos++) {
		for(Int ipos=jpos; ipos<MAX_SHELL_TYPES; ipos++) {
			...
			if (order == 1) {
#ifdef ORDER1
				hgp_os_nai_d1(LCode,np2_,nAtoms_,&braCoePair.front(),&iexp2.front(),&iexpdiff.front(),
						&fbra.front(),&P.front(),A,B,&N.front(),&Z.front(),&result.front());
#endif
			}else {
#ifdef ORDER2
				hgp_os_nai_d2(LCode,np2_,nAtoms_,&braCoePair.front(),&iexp2.front(),&iexpdiff.front(),
						&fbra.front(),&P.front(),A,B,&N.front(),&Z.front(),&result.front());
#endif
			}

			// now let's directly calculate the nai derivatives
				...
				for(Int iDir=0; iDir<derivInfor.getDerivDirLen(); iDir++) {
					...
					for(Int Lj=jLmin; Lj<=jLmax; Lj++) {
						for(Int Li=iLmin; Li<=iLmax; Li++) {
							...
							// now do direct nai
							if (order == 1) {
								directnai_d1(Li,Lj,nAtoms,inp,iCArray,&iexp.front(),A,jnp,jCArray, 
										&jexp.front(),B,&N.front(),&atomicNumbers.front(),pos1,dir1,&abcd.front());
							}else if (order == 2) {
								if (pos1 == pos2 && dir1 == dir2) {
									directnai_d2(Li,Lj,nAtoms,inp,iCArray,&iexp.front(),A,jnp,jCArray, 
											&jexp.front(),B,&N.front(),&atomicNumbers.front(),pos1,dir1,&abcd.front());
								}else{
									directnai_d2(Li,Lj,nAtoms,inp,iCArray,&iexp.front(),A,jnp,jCArray, 
											&jexp.front(),B,&N.front(),&atomicNumbers.front(),pos1,pos2,dir1,dir2,&abcd.front());
								}
							}
