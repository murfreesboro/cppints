...
The basis set functions we discussed here are based on Gaussian 
functions, and for the angular part it could be further 
divided into two groups in terms of its function form.  
One group uses the spherical harmonics for it's angular part:
\begin{equation}
...
In terms of integral calculation, the use of composite shell saves
time on the calculation in vertical recursive relation(VRR)
\footnote{Please refers to \ref{concepts_introduction} for introduction
about VRR}. Because all of sub-shells in the given shell share the same 
exponential factor, and the VRR formula is linear for the contraction
coefficients of $d$; the raw integrals formed in VRR which is without 
contraction coefficients $d$ are all same for all of shell quartets. 
For example, the ket2 shell in shell quartet $(DD|DSP)$ is a composite
shell (one S and one P), and if we did not count in the coefficients of 
$d$ for the SP shell(here S and P have their own $d$) in forming integrals 
used in VRR; the integrals in VRR part are universal for all of shell quartets. 
The result integrals corresponding to $(DD|DS)$ and $(DD|DP)$ can be easily 
retrieved by adding in the contraction $d$ to raw integral:
\begin{align}
 I_{(DD|DS)} &= d_{s}I_{raw} \nonumber \\
 I_{(DD|DP)} &= d_{p}I_{raw}
\end{align}
so they only differ with each other by contraction coefficients. For the more
complicated situation like $(DSP|DSP)$ etc. the above expression becomes:
\begin{align}
 I_{(DS|DS)} &= d_{s,bra2}d_{s,ket2}I_{raw} \nonumber \\
 I_{(DS|DP)} &= d_{s,bra2}d_{p,ket2}I_{raw} \nonumber \\
 I_{(DP|DS)} &= d_{p,bra2}d_{s,ket2}I_{raw} \nonumber \\
 I_{(DP|DP)} &= d_{p,bra2}d_{p,ket2}I_{raw} 
\end{align} 

Because the result integrals for composite shells can be retrieved by adding
contraction coefficients to the result raw integrals, therefore in the VRR 
we have a section named as ``contraction step'' which performs the work showed
above\footnote{please refer to the sqintsprint.cpp for more information}. 
What's more, the procedure described here indicates for VRR it does not
need to contain the contraction information for basis sets to generate VRR 
integrals; therefore all of shell quartets and integrals used in VRR are 
generated without contraction information for the composite shell cases.
Because of this, in the shell class we did not include any composite shell
information so shell class is only defined for non-composite shell situation.
However, in the integral and shell quartet classes, we have the information
defined for composite shell which is named as ``division''. Different pure 
shell quartets in a composite shell quartet like $(DSP|DSP)$ will have its 
own division number to characterize itself. Please refer to sections 
\ref{composite_shell_quartet}, \ref{integral} as well as section \ref{VRR} 
...
Because the ``ONE-TO-ONE'' mapping between the basis set with it's 
corresponding shell\ref{shell}, there's also such correspondence 
between the integral and it's order in corresponding shell quartets. 
...
\begin{align}
 ...
 (P_{z}P_{x}|SS) &\rightarrow  7 \nonumber \\
                 &\cdots
\end{align}
Thus each integral could be re-generated by given it's order number
in the shell quartet.

In integral class we have a data member named as ``division'' which 
is used to represent an ID for integrals inside the composite shell
quartets. For example, for $(DSP|DSP)$ it has four shell quartets:
$(DS|DS)$, $(DS|DP)$, $(DP|DS)$ and $(DP|DP)$ and each one has it's 
own ID number (characterized by division). The integrals inside the 
four shell quartets derive the division number from the corresponding 
shell quartets.
